...
switch($_GET[act]){
  ...
  $tampil=mysql_query("SELECT * FROM basis_pengetahuan ORDER BY id");
	...
	$rgejala=mysql_fetch_array($sql);
       echo "<tr class='".$warna."'>
			 ...
			 <td align=center>$r[bobot]</td>
	          <td><a type='button' class='btn btn-danger margin' href=\"JavaScript: confirmIt('Anda yakin akan menghapusnya ?','$aksi?module=pengetahuan&act=hapus&id=$r[id]','','','','u','n','Self','Self')\" onMouseOver=\"self.status=''; return true\" onMouseOut=\"self.status=''; return true\"><i class='fa fa-trash-o' aria-hidden='true'></i> Hapus</a>
             </td></tr>";
      $no++;
	  $counter++;
    }
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysql_query("SELECT * FROM basis_pengetahuan ORDER BY id limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysql_fetch_array($hasil)){
	...
	$rpenyakit=mysql_fetch_array($sql2);
       echo "<tr class='".$warna."'>
			 ...
			 <td align=center>
	          ...
	          <a type='button' class='btn btn-danger margin' href=\"JavaScript: confirmIt('Anda yakin akan menghapusnya ?','$aksi?module=pengetahuan&act=hapus&id=$r[id]','','','','u','n','Self','Self')\" onMouseOver=\"self.status=''; return true\" onMouseOut=\"self.status=''; return true\">
  ...
  case "tambahpengetahuan":
	echo "	<div class='alert alert-success alert-dismissible'>
                ...
                Silahkan pilih gejala yang sesuai dengan penyakit yang ada, dan berikan <b> nilai bobot </b> dengan cakupan sebagai berikut:<br><br>
				<b>5</b> (Gejala Penting)<br>
				<b>3</b> (Gejala Sedang)<br>
				<b>1</b> (Gejala Ringan)<br>
              </div>
		...
		$hasil4 = mysql_query("SELECT * FROM gejala order by kode_gejala");
		$no=1;
		while($r4=mysql_fetch_array($hasil4)){
			echo "<option value='$r4[kode_gejala]'>G$no&nbsp;&nbsp;|&nbsp;&nbsp;$r4[nama_gejala]</option>";
			$no++;
		}
		echo	"</select></td></tr>
		<tr><td>Bobot</td><td><input autocomplete='off' placeholder='Masukkan Bobot' type=text class='form-control' name='bobot' size=15 ></td></tr>
